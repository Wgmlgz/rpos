...
class MediaService extends SoapService {
      ...
      if (action == '/web/snapshot.jpg') {
        try {
          if (this.ffmpeg_process != null) {
            ...
            var cmd = `ffmpeg -fflags nobuffer -probesize 256 -rtsp_transport tcp -i rtsp://127.0.0.1:${this.config.RTSPPort}/${this.config.RTSPName} -vframes 1  -r 1 -s 640x360 -y snapshot.jpg`;
  ...
  deliver_jpg(response: any){
    try {
      var img = fs.readFileSync('snapshot.jpg');
